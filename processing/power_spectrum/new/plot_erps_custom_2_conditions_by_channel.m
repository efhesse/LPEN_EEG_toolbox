function plot_erps_custom_2_conditions_by_channel(file_name,stats,path_to_save)
...
m = load(file_name);
g = m.g;
P = cellfun(@(x) mean(x,4) ,m.erps,'UniformOutput',false);
R = cellfun(@(x) mean(x,4) ,m.itc,'UniformOutput',false);
freqs = m.freqs;
timesout = m.timesout;

if ~isnan(g.alpha)
    Pboot = cellfun(@(x) mean(x,4) ,m.erpsboot,'UniformOutput',false);
    Rboot = cellfun(@(x) mean(x,4) ,m.itcboot,'UniformOutput',false);
    maskerps = cellfun(@(x) mean(x,4) ,m.maskerps,'UniformOutput',false);
    maskitc = cellfun(@(x) mean(x,4) ,m.maskitc,'UniformOutput',false);
end

mbases = cellfun(@(x) mean(x,3) ,m.mbases,'UniformOutput',false);
mdata = cellfun(@(x) mean(x,3) ,m.mdata,'UniformOutput',false);

resdiff = cellfun(@(x) mean(x,3) ,m.resdiff,'UniformOutput',false);
