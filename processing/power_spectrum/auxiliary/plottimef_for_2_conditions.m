function g = plottimef_for_2_conditions(P, R, Pboot, Rboot, freqs, times, mbase, maskersp, maskitc, g);
%function g = plottimef_for_2_conditions(P, R, Pboot, Rboot, ERP, freqs, times, mbase, maskersp, maskitc, g);
...
%%%%%%%%%%%%%%% plot a topoplot() %%%%%%%%%%%%%%%%%%%%%%%
%
if (~isempty(g.topovec)) && strcmpi(g.plotersp, 'on')
    
%     if strcmp(g.plotersp,'off')
        h(12) = axes('Position',[-.707 .85 .8 .24].*s+q); % place the scalp map at top-left
        h(13) = text(.0,1.01,g.caption);
%     else
